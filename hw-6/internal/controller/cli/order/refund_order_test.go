...
import (
	...
	errsdomain "gitlab.ozon.dev/antonkraeww/homeworks/hw-6/internal/models/domain/errors"
	service "gitlab.ozon.dev/antonkraeww/homeworks/hw-6/internal/service/order"
)

func TestOrderController_RefundOrder(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:    "no_orderid",
			input:   "refund --clientID=1",
			mockFn:  func(f fields) {},
			wantErr: errParseArgs,
		},
		{
			name:    "no_clientid",
			input:   "refund --orderID=1",
			mockFn:  func(f fields) {},
			wantErr: errParseArgs,
		},
		{
			name:  "successful_parse",
			input: "refund --orderID=1 --clientID=1",
			mockFn: func(f fields) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := controller.RefundOrder(ctx, strings.Split(tt.input, " ")[1:])
